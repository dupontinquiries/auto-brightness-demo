...
fn mean(list: &[u8]) -> f64
{
    ...
    // let sum: f64 = list.iter().cloned().sum::<u8>() as f64;
...
pub fn create_brightness_value(offset: i32, data: &[u8]) -> u8
{
    ...
    // println!("middle_half_average = {}", middle_half_average(&data).unwrap());
...
pub fn set_brightness_with_animation(old_brightness: u8, new_brightness: u8, anim_frames: u16, anim_time: u64)
{
    for frame_num in 0..anim_frames - 1 {
        ...
        let intermediate_brightness_value: u8 = value as u8; //( (delta as i16) + (old_brightness as i16) ) as u8;
